...
import com.sixbald.webide.user.UserService;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        return httpSecurity
                ...
                .requestMatchers("/api/**").permitAll() // "/api/"로 시작하는 모든 요청을 모든 사용자에게 허용. 인증되지 않은 모든 사용자가 이 경로에 액세스 가능
//                .antMatchers("/api/", "/api/").permitAll() // 이 두 경로에 대해서도 모든 사용자에게 허용
